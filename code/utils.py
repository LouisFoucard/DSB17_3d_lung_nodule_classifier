...
def s3_upload_big(source_path, chunk_size=50000000):
...
def s3_upload_small(filename):

    creds = pd.DataFrame().from_csv('accessKeys.csv')
    connection = boto.connect_s3(creds['AWSAccessKeyId'][0], creds['AWSSecretKey'][0])
    bucket = connection.get_bucket('louisbucket')

    key = boto.s3.key.Key(bucket, os.path.basename(filename))
    with open(filename) as f:
        print('uploading {}'.format(key.name))
